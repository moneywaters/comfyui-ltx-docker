...
RUN mkdir -p /opt/ComfyUI/models/{diffusion_models,latent_upscale_models,text_encoders,vae,loras} && \
    wget -nv -P /opt/ComfyUI/models/diffusion_models \
    ...
    wget -nv -P /opt/ComfyUI/models/vae \
    ...
    wget -nv -P /opt/ComfyUI/models/text_encoders \
...
RUN echo '#!/bin/bash' > /opt/start.sh && echo 'export PATH=/opt/venv/bin:$PATH' >> /opt/start.sh && \
    echo 'cd /opt/ComfyUI && mkdir -p /workspace/output /workspace/input' >> /opt/start.sh && \
    echo 'python3 main.py --listen 0.0.0.0 --port 8188 --output-directory /workspace/output --input-directory /workspace/input' >> /opt/start.sh && \
