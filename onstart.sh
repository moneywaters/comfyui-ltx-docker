...
API_KEY="${API_KEY:-}"
INSTANCE_ID=$(curl -sf "https://console.vast.ai/api/v0/instances/" -H "Authorization: Bearer ${API_KEY}" 2>/dev/null | python3 -c "
import json, sys
data = json.load(sys.stdin)
items = data if isinstance(data, list) else data.get('instances', [])
for i in items:
    if i.get('public_ipaddr') == '$(hostname -I | awk "{print \$1}")':
        print(i.get('id',''))
" 2>/dev/null || echo "unknown")

echo "Instance: $INSTANCE_ID — $(date)"

# Install buildah and skopeo (don't need podman re-exec)
apt-get update -qq && apt-get install -y -qq buildah skopeo > /dev/null 2>&1
...
# Build the full image using buildah (no re-exec issues)
...
echo "STEP: per-node requirements"
for reqdir in /opt/ComfyUI/custom_nodes/*/requirements.txt; do
  buildah run $c -- bash -c "[ -f \"$reqdir\" ] && pip install -r \"$reqdir\" || true"
done
...
echo "STEP: models (this is the slow part)"
...
# Start script
buildah run $c -- bash -c 'echo "#!/bin/bash" > /opt/start.sh && echo "cd /opt/ComfyUI" >> /opt/start.sh && echo "mkdir -p /workspace/output /workspace/input" >> /opt/start.sh && echo "python3 main.py --listen 0.0.0.0 --port 8188 --output-directory /workspace/output --input-directory /workspace/input" >> /opt/start.sh && chmod +x /opt/start.sh'
...
echo "DONE — docker.io/moneywaters/comfyui-ltx:latest"

if [ "$INSTANCE_ID" != "unknown" ] && [ -n "$INSTANCE_ID" ]; then
    sleep 30
    curl -sf -X DELETE "https://console.vast.ai/api/v0/instances/${INSTANCE_ID}/" -H "Authorization: Bearer ${API_KEY}" || true
fi
